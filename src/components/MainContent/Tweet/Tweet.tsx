...
import type { Tweet, User, Like } from "@prisma/client";
...
type TweetProps = Tweet & { user: User; likes: Like[] };
...
function Body(props: TweetProps) {
    return (
        <div className="pl-16">
...
function Icons(props: TweetProps) {
    let { data } = useSession();
    ...
    let liked = false,
        retweeted = false,
        replied = false;
    if (props.likes.map((l) => l.userId === data?.userData.id)) {
    }
    ...
    return (
        <div className="flex items-center">
            <div
                onClick={reply}
                className="duration-350 flex flex-1 items-center text-xs text-gray-800 transition ease-in-out hover:text-blue-400 dark:text-white dark:hover:text-blue-400"
            >
